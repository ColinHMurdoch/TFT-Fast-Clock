...
/*
 * Fast Clock Version 2.0
 * 
 * Written by Jim Gifford June 2017
 ...
 * Modified to opere on an Arduino Uno with an McuFriend type TFT LCD Shield.
 *
 * Design Notes:
 *
 ...
 * 
 * The system can now use the EEPROM to hod the time value.  If Button 2 (Save) is prwssed, the clock is halted 
...
// Load the specil font for the clock display - 24 point wont load
...
void showmsgXY(int x, int y, int sz, char colour, const char *msg)
{
    int16_t x1, y1;
    uint16_t wid, ht;
    //tft.fillRect(1, y, 240, y+15, BLACK);
...
void TFT_Begin()
{
    ...
    //showmsgXY(130, 10, 1, YELLOW, "Junction");
...
// This is the print section.  Change this to print to TFT display.
    ...
    tft.setTextSize(1);
    tft.fillRect(1, 30, 235, 100, BLACK);
    //delay(10);
    tft.setCursor(1,100);
   ...
   if (!pausePlay){
     PauseClock();
        ...
        HH = ((startTime + runTime) / milPerHr) ;
          ...
          Time = Hour += MinuteS += '\0';  //  Add null for dispaly
          Time.toCharArray(message, 8);
          printClock(message);
